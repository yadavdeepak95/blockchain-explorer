...
class CRUDService {
  ...
  getTxCountByBlockNum(channelName, blockNum) {
    return sql.getRowByPkOne(
      `select blocknum ,txcount from blocks where genesis_block_hash='${channelName}' and blocknum=${blockNum} `
    );
  }
  ...
  async getChannelConfig(channelName) {
    let channelConfig = await sql.getRowsBySQlCase(
      ` select * from channel where name ='${channelName}' `
    );
  ...
  async saveBlock(block) {
    let c = await sql.getRowByPkOne(`select count(1) as c from blocks where blocknum='${
      block.blocknum
        ...
        and genesis_block_hash='${block.genesis_block_hash}' and prehash='${
      block.prehash
    }' and datahash='${block.datahash}' `);
    if (c.c == 0) {
      await sql.saveRow('blocks', block);
      await sql.updateBySql(
        `update channel set blocks =blocks+1 where genesis_block_hash='${
  ...
  async saveTransaction(transaction) {
    let c = await sql.getRowByPkOne(
      `select count(1) as c from transactions where blockid='${
        transaction.blockid
      }' and txhash='${transaction.txhash}' and genesis_block_hash='${
        transaction.genesis_block_hash
      }'`
    );
    ...
    if (c.c == 0) {
      ...
      await sql.updateBySql(
        `update chaincodes set txcount =txcount+1 where genesis_block_hash='${
          transaction.genesis_block_hash
        }'`
      );
      await sql.updateBySql(
        `update channel set trans =trans+1 where genesis_block_hash='${
          transaction.genesis_block_hash
        }'`
      );
      return true;
  ...
  async getCurBlockNum(channelName) {
    try {
      var row = await sql.getRowsBySQlCase(
        `select max(blocknum) as blocknum from blocks  where genesis_block_hash='${channelName}'`
      );
    } catch (err) {
      logger.error(err);
      return -1;
  ...
  async saveChaincode(chaincode) {
    let c = await sql.getRowByPkOne(
      `select count(1) as c from chaincodes where name='${
        chaincode.name
      }' and genesis_block_hash='${
        chaincode.genesis_block_hash
  ...
  async savePeer(peer) {
    let c = await sql.getRowByPkOne(
      `select count(1) as c from peer where genesis_block_hash='${
