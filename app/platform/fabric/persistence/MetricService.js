...
class MetricService {
  ...
  getChaincodeCount(channelName) {
    return sql.getRowsBySQlCase(
      `select count(1) c from chaincodes where genesis_block_hash='${channelName}' `
    );
  }

  getPeerlistCount(channelName) {
    return sql.getRowsBySQlCase(
      `select count(1) c from peer where genesis_block_hash='${channelName}' `
    );
  }

  getTxCount(channelName) {
    return sql.getRowsBySQlCase(
      `select count(1) c from transactions where genesis_block_hash='${channelName}'`
    );
  }

  getBlockCount(channelName) {
    return sql.getRowsBySQlCase(
      `select max(blocknum) c from blocks where genesis_block_hash='${channelName}'`
    );
  }
  ...
  async getOrdererData() {
    let ordererArray = [];
    var c1 = await sql.getRowsBySQlNoCondtion(
      `select c.requests as requests,c.server_hostname as server_hostname,c.genesis_block_hash as genesis_block_hash from orderer c`
    );
    for (var i = 0, len = c1.length; i < len; i++) {
      var item = c1[i];
      ordererArray.push({
