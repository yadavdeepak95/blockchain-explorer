...
var updateAnchorPeers = async function(
  channelName,
  configUpdatePath,
  username,
  org_name
  ...
  try {
    ...
    event_hubs.forEach(eh => {
      let anchorUpdateEventPromise = new Promise((resolve, reject) => {
        ...
        eh.registerBlockEvent(
          block => {
            logger.info(
              'The config update has been committed on peer %s',
              eh.getPeerAddr()
            );
    ...
    if (response) {
      if (response.status === 'SUCCESS') {
        logger.info(
          'Successfully update anchor peers to the channel %s',
          channelName
        );
      } else {
        error_message = util.format(
          'Failed to update anchor peers to the channel %s with status: %s reason: %s',
          channelName,
          response.status,
          response.info
        );
      ...
      error_message = util.format(
        'Failed to update anchor peers to the channel %s',
        channelName
      );
      logger.error(error_message);
    }
    ...
    logger.error(
      'Failed to update anchor peers due to error: ' + error.stack
        ? error.stack
        : error
    );
  ...
  if (!error_message) {
    ...
    let message = util.format(
      'Failed to update anchor peers. cause:%s',
      error_message
    );
    logger.error(message);
    const response = {
