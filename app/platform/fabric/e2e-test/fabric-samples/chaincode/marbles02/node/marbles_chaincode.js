...
let Chaincode = class {
  ...
  async initMarble(stub, args, thisClass) {
    ...
    let colorNameIndexKey = await stub.createCompositeKey(indexName, [
      marble.color,
      marble.name
    ]);
  ...
  async delete(stub, args, thisClass) {
    if (args.length != 1) {
      throw new Error(
        'Incorrect number of arguments. Expecting name of the marble to delete'
      );
    }
    let marbleName = args[0];
    if (!marbleName) {
    ...
    // delete the index
    let indexName = 'color~name';
    let colorNameIndexKey = stub.createCompositeKey(indexName, [
      marbleJSON.color,
      marbleJSON.name
    ]);
  ...
  async transferMarble(stub, args, thisClass) {
    ...
    if (args.length < 2) {
      throw new Error(
        'Incorrect number of arguments. Expecting marblename and owner'
      );
    }
  ...
  async transferMarblesBasedOnColor(stub, args, thisClass) {
    ...
    if (args.length < 2) {
      throw new Error(
        'Incorrect number of arguments. Expecting color and owner'
      );
    }
    ...
    let coloredMarbleResultsIterator = await stub.getStateByPartialCompositeKey(
      'color~name',
      [color]
    );
    ...
    while (true) {
      ...
      console.info(
        util.format(
          '- found a marble from index:%s color:%s name:%s\n',
          objectType,
          returnedColor,
          returnedMarbleName
        )
    ...
    let responsePayload = util.format(
      'Transferred %s marbles to %s',
      color,
      newOwner
    );
  ...
  async getMarblesByRangeWithPagination(stub, args, thisClass) {
    ...
    const { iterator, metadata } = await stub.getStateByRangeWithPagination(
      startKey,
      endKey,
      pageSize,
      bookmark
